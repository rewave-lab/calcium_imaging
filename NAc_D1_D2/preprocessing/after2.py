...
for i in os.listdir(fnames[0][:-5]+"_maybe"):
    resting_components.append(int(i[3:6]))
...
import pickle
with open(fnames[0]+"_DF_filtered_all.txt","wb") as fp:
    pickle.dump(np.array(denoised_z_traces),fp)

plt.rcParams['figure.facecolor'] = 'white'
new_folder = fnames[0][:-5]+'_filtered_all'
...
print("ok!")
import pickle
with open(fnames[0]+"_A_filtered_all.txt","wb") as fp:
    pickle.dump(cnm.estimates.A[:,idx],fp)


with open(fnames[0]+"_C_filtered_all.txt","wb") as fp:
...
plt.savefig('filtered_components_all_'+fnames[0][:-5]+'.png',dpi= 300)
plt.close()

import matplotlib.image as mpimg
img = mpimg.imread('components_'+fnames[0][:-5]+'.png')
img2 = mpimg.imread('filtered_components_all_'+fnames[0][:-5]+'.png')
...
plt.tight_layout()
plt.savefig('filtered_components_all_compare_'+fnames[0][:-5]+'.png', dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig('filtered_lights_all_'+fnames[0][:-5]+'.png',dpi= 300)
plt.close()

img = mpimg.imread('lights_'+fnames[0][:-5]+'.png')
img2 = mpimg.imread('filtered_lights_all_'+fnames[0][:-5]+'.png')
...
plt.tight_layout()
plt.savefig('filtered_lights_all_compare_'+fnames[0][:-5]+'.png', dpi=300)
plt.close()
